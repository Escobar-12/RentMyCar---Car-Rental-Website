...
const AdminCarCard = ({ car, setShowModal, setCarToDelete }) => {
    const [expanded, setExpanded] = useState(false);
    const [isApprouved, setIsApprouved] = useState(car.isApprouved);
    const {token} = useAuth();
    const {fetchWithToken} = useApplication();

    const setCarApprouved = async() =>
    {
        try
        {
            const res = await fetchWithToken(`http://localhost:5007/car/approuveCar`,
                {
                method:"PUT",
                body: JSON.stringify({isApprouved:!isApprouved,carId:car._id}),
                headers: 
                    {
    ...
    return (
        <div className={` min-w-80 max-w-85 sm:w-85 group rounded-xl overflow-hidden shadow-lg cursor-pointer transition-all duration-500 ${expanded ? "h-120" : "h-84"}`} >
            ...
            <div className='mt-5 px-4 sm:px-5 w-full flex items-center gap-4'>
                <ToggleSwitch checked={isApprouved} onChange={() => setCarApprouved()} />
                <p className='font-semibold text-lg'>Car Approuved</p>
            </div>
            <div className='mt-5 px-4 sm:px-5 w-full flex justify-end items-center gap-4'>
                <div onClick={() => handleRemove()} className='p-3 bg-red-400/80 text-white text-lg font-semibold rounded-lg hover:bg-red-500 hover:text-xl transition-all duration-200'>Remove Car</div>
            </div>
        </div>
    );
};
